...
filtered_data = pd.read_csv('Final_Data_PV_Clear_Loadshedding.csv')

loadshedding = filtered_data['Loadshedding']
...
stove_energy = filtered_data['Stove E']
geyser_energy = filtered_data['Geyser E']
light_energy = filtered_data['Lights and Pool E']
...
for i in range(len(filtered_data)):
    ...
    # For subsequent time_stamps
        ...
        end_time2 = pd.to_datetime('20:30:00').time()           # End2-Start1 for nights
        start_time3 = pd.to_datetime('05:30:00').time()         # Start3-Start2 for excess
        
        #-------------Load Shedding Not Present-----------------        
        if 'no' in loadshedding[i]:
            ...
            if required_energy.iloc[i] > 0:
                ...
                if start_time1 <= current_time < end_time1:
                    ...
                    #-------------Scenario 1: Battery can supply in time-----------------
                    # Battery capable of supplying within limits
                    if battery_SoC_min <= SoC[i-1] <= battery_SoC_max and state_priority_battery == True:
                        ...
                        if state_priority_battery == True:
                            # Get battery charge and discharge
                            battery_charging_pv[i] = excess_energy[i] * n_charge
                            # Assign charge or discharge energy values                   
                            battery_charging[i] = battery_charging_grid[i] + battery_charging_pv[i]
                            if required_energy[i] > 0:
                                battery_discharge[i] = required_energy[i]/n_discharge - excess_energy[i] * n_charge      # If load greater than pv, discharge battery (excess probably 0)
                            # Check that charge does not exceed 1C charge
                            elif battery_charging[i] > battery_power_plot_max(SoC[i-1]) * n_charge * 0.5:
                            ...
                            # Calculate battery energy, note no grid in morning due to night charge of battery
                            battery_energy[i] = battery_energy[i-1] - required_energy[i]/n_discharge + excess_energy[i] * n_charge          # Excess = (PV-Load), if < 0 then = 0
                            # Check if battery energy is greater than battery capacity, if yes, assign to grid
                            if battery_energy[i] > battery_capacity:
                                grid_energy[i] = battery_capacity - battery_energy[i]   # Pushed to the grid if negative, postive is when needed
                                battery_energy[i] = battery_capacity
                            elif battery_energy[i] < 0:
                                if battery_discharge[i] > battery_energy[i-1]:
                                    battery_discharge[i] = battery_energy[i-1]
                                    grid_energy[i] = required_energy[i] - battery_discharge[i]                       # If battery is over push excess to grid
                                #grid_energy[i] = grid_energy[i] - battery_energy[i]                        # Will be postive if energy is drawn
                                #battery_discharge[i] = battery_discharge[i] - grid_energy[i]               # If battery is over push excess to grid
                                battery_energy[i] = 0
                            else:
                            ...
                            if SoC[i] < 0:
                                ...
                                state_priority_grid = False
                                state_check = True
                            elif battery_SoC_max > SoC[i] >= battery_SoC_initial:
                                state_priority_battery = True
                                state_priority_grid = False
                                state_check = True                            
                            ...
                            if battery_voltage[i] < battery_voltage_plot1(0):
                                ...
                                battery_voltage[i] = battery_voltage_plot1(SoC[i]) 
                            '''   
                            # Assign charge or dischage energy values
                            if load_energy[i]/n_discharge > excess_energy[i] * n_charge:
                                battery_discharge[i] = load_energy[i]/n_discharge - excess_energy[i] * n_charge
                            else:
                                # Check that charging does not exceed battery max amp ratings
                                battery_charging[i] = excess_energy[i] * n_charge - load_energy[i]/n_discharge
                                if battery_charging[i] > battery_power_plot_max(SoC[i]) * n_charge * 0.5:      # Check that charge does not exceeds 1C, 0.5 is to account for energy in 30 mins
                                    battery_charging[i] = battery_power_plot_max(SoC[i]) * n_charge * 0.5      # If it does, set to 1C
                                else:
                                    battery_charging[i] = battery_charging[i]
                                    battery_charging_pv[i] = battery_charging[i]                                # Assign all charge form PV as grid not suppose to charge
                            '''        
                    
                    #-------------Scenario 2: Battery can not supply in time-----------------                    
                    # Battery out of bounds, need to charge plus supply load form grid and pv
                    elif state_priority_grid == True and SoC[i-1] <= battery_SoC_initial:
                        # Calculate battery energy but first charging required, note grid used due to battery unable to supply load
                        if loadshedding[i+2] == 'yes':
                            battery_charging_grid[i] = battery_power_plot_max(SoC[i-1]) * n_charge * 0.5
                        ...
                        # Calculate grid energy meaning load and charge
                ...
                # Check if the current time is within the specified range for day time
                    ...
                    #-------------Scenario 1: Battery can supply in time-----------------    
                    # Check when SoC is less than minimum SoC
                    # Battery capable of supplying within limits
                    if battery_SoC_min < SoC[i-1] <= battery_SoC_max and state_priority_battery == True:
                        # Check if states are changed
                        if state_check == True:                    
                        ...
                        # Check battery information
                        if state_priority_battery == True:
                            # Calculate battery energy but first charging required, note pv prioritised on clear sky days
                            # No grid charging
                            battery_charging_pv[i] = excess_energy[i] * n_charge                                    # Excess energy is used as pv charging
                            # Assign charge or discharge energy values                   
                            battery_charging[i] = battery_charging_pv[i]
                            if required_energy[i] > 0:
                                battery_discharge[i] = required_energy[i]/n_discharge - excess_energy[i] * n_charge     # If load greater than pv, discharge battery (excess probably 0)
                            # Check that charge does not exceed 1C charge
                            elif battery_charging[i] > battery_power_plot_max(SoC[i-1]) * n_charge * 0.5:
                                ...
                                battery_charging[i] = battery_charging[i]
                            # Calculate battery energy
                            battery_energy[i] = battery_energy[i-1] - required_energy[i]/n_discharge + battery_charging[i]    
                            # Calculate grid energy meaning only pushback, no load
                            grid_energy[i] = battery_charging_grid[i]
                            # Check if battery energy is greater than battery capacity, if yes, assign to grid
                            if battery_energy[i] > battery_capacity:
                                grid_energy[i] = grid_energy[i] - (battery_energy[i] - battery_capacity)   # If battery is over push excess to grid   
                                battery_charging[i] = battery_charging[i] - (battery_energy[i] - battery_capacity)   
                            ...
                            if SoC[i] <= 0:
                                ...
                                state_priority_grid = True
                                state_check = False     
                            elif SoC[i] >= battery_SoC_max:
                                SoC[i] = 100
                                state_priority_battery = True
                                state_priority_grid = False
                                state_check = True 
                            elif battery_SoC_min < SoC[i] < battery_SoC_max:
                                state_priority_battery = True
                                state_priority_grid = False
                                state_check = True
                            elif SoC[i] <= battery_SoC_min:
                    ...
                    # Check when SoC is less than minimum SoC
                    # Battery capable of supplying within limits
                    elif SoC[i-1] <= battery_SoC_min or state_priority_grid == True:
                        # Check if states are changed
                        if state_check == True:                    
                            state_priority_battery = False
                            state_priority_grid = True
                ...
                #-------------Evenings-----------------
                # Check if the current time is within the specified range for day time
                if start_time2 <= current_time < end_time2:
                    ...
                    #-------------Scenario 2: Battery can not supply in time-----------------                    
                    # Battery out of bounds, need to charge plus supply load form grid and pv
                    elif SoC[i-1] <= battery_SoC_initial or state_priority_grid == True:   
                        if loadshedding[i+2] == 'yes':
                            # Calculate battery energy but first charging required, note grid used due to battery unable to supply load
                            battery_charging_grid[i] = battery_power_plot_max(SoC[i-1]) * n_charge * 0.5
                            if battery_charging_grid[i] < 0:
                ...
                #-------------Night-----------------
                # Check if the current time is within the specified range for day time
                if end_time2 <= current_time or current_time < start_time1:
            ...
            if excess_energy.iloc[i] > 0:
                ...
                # Check if the current time is within the specified range for day time
                if start_time3 <= current_time <= start_time2:
        ...
        #-------------Load Shedding Present-----------------                    
        elif 'yes' in loadshedding[i]:
            ...
            if required_energy.iloc[i] > 0:
                ...
                if start_time1 <= current_time < end_time1:
                    ...
                    #-------------Scenario 1: Battery has to supply load----------------
                    # Battery capable of supplying within limits
                    if SoC[i-1] <= battery_SoC_max:
                        # Check if states are changed
                        if state_check == True:
                            state_priority_battery = True
                            state_priority_grid = False
                        ...
                        if 1 == 1:
                            ...
                            # Assign charge or discharge energy values                   
                            battery_charging[i] = battery_charging_grid[i] + battery_charging_pv[i]
                            # Ajust load requirements due to load shedding
                            if 45 < SoC[i-1] <= 65:
                                required_energy[i] = required_energy[i] - geyser_energy[i]                              # If battery is in the middle of the SoC range, then disgard geyser energy
                            elif 15 < SoC[i-1] <= 45:
                                required_energy[i] = required_energy[i] - stove_energy[i] - geyser_energy[i]            # If battery is in the lower middle of the SoC range, then switch off geyser energy and stove
                            elif SoC[i-1] <= 15:
                                required_energy[i] = required_energy[i] - stove_energy[i] - geyser_energy[i] - light_energy[i]  # If battery is in the lower range of the SoC, then switch off geyser energy and stove and lights
                            else:
                                required_energy[i] = required_energy[i]
                            # Get battery discharge based on required energy above
                            if required_energy[i] > 0:
                                battery_discharge[i] = required_energy[i]/n_discharge - excess_energy[i] * n_charge      # If load greater than pv, discharge battery (excess probably 0)
                            # Check that charge does not exceed 1C charge
                            elif battery_charging[i] > battery_power_plot_max(SoC[i-1]) * n_charge * 0.5:
                ...
                # Check if the current time is within the specified range for day time
                    ...
                    if SoC[i-1] <= battery_SoC_max:
                        ...
                        if 1 == 1:
                            ...
                            # Calculate battery energy
                            battery_energy[i] = battery_energy[i-1] - required_energy[i]/n_discharge + battery_charging[i]    
                            # Calculate grid energy = 0 because grid away in load shedding
                            grid_energy[i] = 0
                            # Check if battery energy is greater than battery capacity, if yes, assign to grid
                            if battery_energy[i] > battery_capacity:
                                battery_charging[i] = battery_charging[i] - (battery_energy[i] - battery_capacity)   
                    ...
                    if SoC[i-1] <= battery_SoC_max:
                        ...
                        if 1 == 1:
                            ...
                            if battery_energy[i] > battery_capacity:
                                ...
                                battery_discharge[i] = battery_energy[i-1]               # If battery is over push excess to grid
                                battery_energy[i] = 0
                            else:
                                battery_energy[i] = battery_energy[i]
                            # Calculate SoC
...
data_to_append.to_csv('Final_Data_Bat_3B.csv', index=False, header=True)
