...
for i in range(len(filtered_data)):
    ...
    # For subsequent time_stamps
        ...
        start_time1 = pd.to_datetime('06:30:00').time()         # Start1-End1 for monrings
        end_time1 = pd.to_datetime('9:30:00').time()            # End1-Start2 for midday
        start_time2 = pd.to_datetime('17:30:00').time()         # Start2-End2 for evenings
        end_time2 = pd.to_datetime('20:00:00').time()           # End2-Start1 for nights
        ...
        if 'no' in loadshedding[i]:
            ...
            if required_energy.iloc[i] > 0:
                ...
                if start_time1 <= current_time < end_time1:
                    ...
                    #-------------Scenario 1: Battery discharge and Grid supply-----------------
                    ...
                    if battery_SoC_min < SoC[i-1] <= battery_SoC_max and state_priority_battery == True:
                        ...
                        if state_priority_battery == True:
                            ...
                            if required_energy[i] > 0:
                                battery_discharge[i] = 1024     # Discharge according to 0.5C curve
                            ...
                            battery_energy[i] = battery_energy[i-1] - battery_discharge[i] + excess_energy[i] * n_charge          # Battery only discharging and recieveing excess              
                            ...
                            if battery_energy[i] > battery_capacity:
                                ...
                                if battery_discharge[i] > battery_energy[i-1]:
                                    ...
                                    grid_energy[i] = required_energy[i] - battery_discharge[i] * n_discharge                      # If battery is over push excess to grid
                                ...
                                grid_energy[i] = required_energy[i] + battery_charging_grid[i]/n_charge - battery_discharge[i] * n_discharge
                    ...
                    elif state_priority_grid == True or SoC[i-1] <= battery_SoC_initial:
                ...
                # Check if the current time is within the specified range for day time
                    ...
                    #-------------Scenario 2: Battery can not supply in time-----------------                    
                    # Battery out of bounds, need to charge plus supply load form grid and pv
                    elif SoC[i-1] <= battery_SoC_initial and state_priority_grid == True:   
                        # Calculate battery energy but first charging required, note grid used due to battery unable to supply load
                        battery_charging_grid[i] = battery_power_plot1(SoC[i-1]) * n_charge * 0.5
                        if battery_charging_grid[i] < 0:
                            battery_charging_grid[i] = 0
                ...
                #-------------Night-----------------
                # Check if the current time is within the specified range for day time
                elif end_time2 <= current_time or current_time < start_time1:
                    #-------------Scenario 1: Grid to charge battery and supply load-----------------  
                    # Check if states are changed
                    # Calculate battery energy but first charging required, note grid used due to battery unable to supply load
                    battery_charging_grid[i] = battery_power_plot1(SoC[i-1]) * n_charge * 0.5
            ...
            if excess_energy.iloc[i] > 0:
                ...
                #------------- Mornings-----------------
                # Check if the current time is within the specified range for day time
                if start_time3 <= current_time <= end_time1:
                    ...
                    battery_charging_pv[i] = excess_energy[i] * n_charge                            # Excess energy is used as pv charging
                    # Assign charge energy, note only form PV and discharge should be 1024Wh per 30 minutes
                    if start_time3 <= current_time <= end_time1 and 39 < SoC[i-1]:
                        battery_discharge[i] = 1024
                    else:
                        battery_discharge[i] = 0                   
                    ...
                    # Calculate battery energy
                    battery_energy[i] = battery_energy[i-1] + battery_charging[i]  - battery_discharge[i]                 # Takes into account excess, make sure not exceeding charge limit plus load supplied by pv and makes sure discharge is present
                    # Calculate grid energy meaning only excess energy being pushed back
                    grid_energy[i] = battery_charging_grid[i]/n_charge - battery_discharge[i] * n_discharge
                    # Check if battery energy is greater than battery capacity, if yes, assign to grid
                    if battery_energy[i] > battery_capacity:
                        grid_energy[i] = grid_energy[i] - (battery_energy[i] - battery_capacity)   # If battery is over push excess to grid   
                        battery_charging[i] = battery_charging[i] - (battery_energy[i] - battery_capacity)                    
                ...
                if start_time1 <= current_time < end_time1:
                    ...
                    if SoC[i-1] <= battery_SoC_max:
                        ...
                        if 1 == 1:
                            ...
                            battery_charging[i] = battery_charging_grid[i] + battery_charging_pv[i]
                            if required_energy[i] > 0:
                                battery_discharge[i] = required_energy[i]/n_discharge - excess_energy[i] * n_charge   # Battery needs to supply load
                            # Check that charge does not exceed 1C charge
                            elif battery_charging[i] > battery_power_plot_max(SoC[i-1]) * n_charge * 0.5:
                                battery_charging[i] = battery_power_plot_max(SoC[i-1]) * n_charge * 0.5     # If it does, set to 1C
                                battery_charging_grid[i] = battery_charging[i] - battery_charging_pv[i]     # If charge greater than 1C, change grid value to use full pv charge, negative is push back
                ...
                # Check if the current time is within the specified range for day time
                    ...
                    if SoC[i-1] <= battery_SoC_max:
                        ...
                        if 1 == 1:
                            ...
                            # Gird energy 0 because no grid import or export
                            grid_energy[i] = 0
                            # Check if battery energy is greater than battery capacity, if yes, assign to grid
                            if battery_energy[i] > battery_capacity:  
                                battery_charging[i] = battery_charging[i] - (battery_energy[i] - battery_capacity)   
                ...
                # Check if the current time is within the specified range for day time
                    ...
                    if SoC[i-1] <= battery_SoC_max:
                        ...
                        if 1 == 1:
                            ...
                            battery_charging[i] = battery_charging_grid[i] + battery_charging_pv[i]
                            if required_energy[i] > 0:
                                battery_discharge[i] = required_energy[i]/n_discharge - excess_energy[i] * n_charge     # Battery has to supply the load
                            # Check that charge does not exceed 1C charge
                            elif battery_charging[i] > battery_power_plot_max(SoC[i-1]) * n_charge * 0.5:
                                battery_charging[i] = battery_power_plot_max(SoC[i-1]) * n_charge * 0.5     # If it does, set to 1C
                                battery_charging_grid[i] = battery_charging[i] - battery_charging_pv[i]     # If charge greater than 1C, change grid value to use full pv charge, negative is push back
...
data_to_append.to_csv('Final_Data_Bat_3A.csv', index=False, header=True)
